...
>>>>>>> 4525adace58521e0274c7ca25638de15e5b67410
...
//This demo only works on OV5640_MINI_5MP_PLUS or OV5642_MINI_5MP_PLUS platform.
//Make sure to define the right one
...
void myCAMSaveToSDFile(ArduCAM myCAM) {
  ...
  Serial.println(F("Capture Done!"));
  length = myCAM.read_fifo_length();
  Serial.print(F("The read FIFO size is: "));
  Serial.println(length, DEC);
  if (length >= MAX_FIFO_SIZE || length == 8388607) //8M
  {
    Serial.println(F("Image bigger than the FIFO or no camera connected."));
    return 0;
  } else if (length == 0 ) { //Nothing was saved
    Serial.println(F("Nothing was captured"));
    return 0;
  }
  //Construct a file name
  k = k + 1;
  ...
  outFile = SD.open(str, O_WRITE | O_CREAT | O_TRUNC);
  if (!outFile) {
    Serial.println(F("Opening file for write failed"));
    return;
  }
  myCAM.CS_LOW();
  myCAM.set_fifo_burst();
  while ( length-- )
  {
    ...
    if ( (temp == 0xD9) && (temp_last == 0xFF) ) //If find the end ,break while,
    {
      ...
      //Close the file
      outFile.close();
      Serial.println(F("Image save completed."));
      is_header = false;
      i = 0;
    }
    if (is_header == true)
