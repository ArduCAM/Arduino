...
void setup() {
 ...
 Serial.println("ArduCAM Start!"); 
  ...
  if(temp != 0x55)
  {
    Serial.println("ACK CMD SPI2 interface Error!");
    cam2 = false;
  }
  ...
  if(temp != 0x55)
  {
    Serial.println("ACK CMD SPI3 interface Error!");
    cam3 = false;
  }
  ...
  if(temp != 0x55)
  {
    Serial.println("ACK CMD SPI4 interface Error!");
    cam4 = false;
  }
 #if defined (OV5640_MINI_5MP_PLUS)
...
void loop() {
 ...
 if (Serial.available())
 {
  ...
  switch(temp)
  {
    ...
    case 0x15: 
      if(cam1){
      Serial.println("ACK CMD CAM1 start Capture"); 
      myCAM1.flush_fifo(); 
      myCAM1.clear_fifo_flag();   
      myCAM1.start_capture(); 
    }
    if(cam2){
      Serial.println("ACK CMD CAM2 start Capture"); 
      myCAM2.flush_fifo(); 
      myCAM2.clear_fifo_flag();   
      myCAM2.start_capture();  
    }
    if(cam3){
      Serial.println("ACK CMD CAM3 start Capture"); 
      myCAM3.flush_fifo();
      myCAM3.clear_fifo_flag();   
      myCAM3.start_capture();   
    } 
    if(cam4){
      Serial.println("ACK CMD CAM4 start Capture"); 
      myCAM4.flush_fifo();  
      myCAM4.clear_fifo_flag();   
  ...
  if(count >= 1)
  {
    ...
    {
      count--;
      Serial.println("ACK CMD CAM1 Capture Done!");
    ...
    {
       count--;
      Serial.println("ACK CMD CAM2 Capture Done!");
    ...
    {
        count--;
      Serial.println("ACK CMD CAM3 Capture Done!");
    ...
    {
        count--;
      Serial.println("ACK CMD CAM4 Capture Done!");    
